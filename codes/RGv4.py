...
It stores the relative errors for each configuration.
"""
...
# List of step sizes for the algorithm
...
for n_local_step in tqdm(N_LOCAL_STEP):
    ...
    for _ in tqdm(range(N_TRIAL)):
        ...
        lr_x = 1 / (ell.item() * n_local_step + 2 * (n_local_step - 1) * Lmax.item() * np.sqrt(ell.item() / mu.item()))  # For minimizing x_i
        ...
        for _ in range(N_COMM):
            ...
            trial_errors.append(game.opt_dist(q) / init_dist)
...
with open('results/RGv4.pkl', 'wb') as f:
